...
## 2026-09-03 - Work Package 2.24 Installable iPhone and iPad Home Screen Experience (Start)

- Started the installable-app work in isolated worktree `/tmp/glam-pwa.YGMcEa` on `codex/installable-studio-app-v2`, based on the current production `main` after the completed Service Menu release.
- The earlier combined implementation is retained only as a source reference on `codex/installable-studio-app-wip`; Service Menu code, API defaults, and print rules will not be copied back into this independent workstream.
- Current WebKit guidance confirms that manifest `display: standalone` opens a Home Screen web app separately from the browser, while `viewport-fit=cover` and the four `safe-area-inset-*` values are the appropriate web layout controls for device cutouts and system areas.
- Acceptance includes base-path-safe metadata and icons, app-like iPhone/iPad layouts, public-shell-only service-worker caching, no installation guide, local and production-base validation, and a real-iPhone Add to Home Screen/manual-use gate before merge.
- The first focused frontend typecheck in the fresh worktree failed because the generated `@workspace/api-client-react` declaration output had not yet been built there (`TS6305`). This is a worktree setup-order failure; the service-worker syntax check and both root-base and production-base frontend builds completed. Generated dependencies will be built first and the typecheck rerun sequentially.
- The first production-preview command forwarded `--port 4174` after an extra argument separator, so Vite ignored it and reported that the already-visible development server owns port 5173. No process or project file was changed; the preview will run directly from the frontend workspace on port 4174.
- The next preview started on port 4174 but omitted `BASE_PATH` at serve time. Requests below `/YeasminGlamDashboard/` returned HTTP 200 with `text/html` fallbacks instead of each asset's real content type, so that probe was rejected. The preview will be restarted with the production base path and every asset rechecked.
- Raw headless Chrome screenshots at a requested 430-pixel window initially appeared to clip the login card because that command-line path retained a wider desktop layout viewport. An exact DevTools device-metrics check is being used instead. Its first run correctly measured the page but failed only because Chrome did not honor the attempted `display-mode: standalone` media-feature emulation; standalone behavior remains a real-iPhone gate, while exact viewport, overflow, card bounds, and input sizing will be rerun without that unsupported assertion.
- The exact 430-pixel device-metrics rerun found the login input still computed to `15px` because its component utility class outranked the general mobile element rule. The phone-only input/select/textarea rule will explicitly win the cascade so Safari does not zoom editable fields on focus.

## 2026-09-03 - Work Package 2.24 Installable Experience (Validated Locally)

- Reconstructed the installable-app changes cleanly on current `main`, without the earlier branch's stale Service Menu API wording or unrestricted print-page rule. The application diff is limited to install metadata/icons, service-worker registration and policy, shared safe-area/touch CSS, theme-color synchronization, and the Website Studio safe-area class.
- Added a base-path-safe manifest with standalone display and branded Apple touch, 192, 512, and maskable 512 pixel icons. Direct dimension inspection matches every declaration, and the 512-pixel artwork retains full burgundy coverage with centered ivory `YB` lettering and safe interior padding.
- Both the local-root and `/YeasminGlamDashboard/` production builds completed. Preview probes returned the correct HTTP 200 content types for the HTML, manifest, service worker, favicon, Apple touch icon, and every manifest icon at both base paths.
- The service-worker routing harness passed eight cases: shell navigation, hashed assets, and public Service Menu images are eligible for handling; same-origin API, shared `/glam-api/api/`, cross-origin, POST, and unknown private-looking paths are bypassed.
- A real production-style Chrome registration check confirmed scope `/YeasminGlamDashboard/`, cache `glam-studio-shell-v1`, hashed JavaScript/CSS shell files, manifest, favicon, and icons. No cached URL contained `/api/`. With network emulation switched fully offline, a controlled reload still rendered the cached `Glam Studio` sign-in shell; the worker never force-reloads an open page.
- Exact DevTools mobile metrics passed at 430-by-932 iPhone portrait, 932-by-430 landscape, and 820-by-1180 iPad portrait: zero horizontal overflow, bounded login card, and stable content bounds. The phone login and authenticated booking form inputs compute to 16 pixels after fixing the initial 15-pixel override.
- Authenticated development checks passed without production writes: the 430-pixel dashboard header and bottom navigation remained inside the viewport, an unsaved booking text input accepted focus and local text without submission, and Party Services rendered with bounded main content at landscape and iPad widths.
- The in-app Browser now exposes two visible previews: the production-base installable sign-in surface at port 4174 and the authenticated app-like dashboard at port 5174. The Browser sandbox does not expose its service-worker navigator API; service-worker runtime proof comes from the separate Chrome registration/cache inspection.
- Final typecheck, root-base build, production-base build, service-worker syntax, manifest/icon/HTTP probes, privacy routing harness, device-metrics checks, and `git diff --check` pass. Existing sourcemap, OpenCV externalization, and large-chunk warnings remain unchanged.
- This branch remains unmerged. Real-iPhone Add to Home Screen, standalone launch, sign-in, safe-area, background/resume, and update acceptance are still required before Work Package 2.24 can be marked complete or merged.
- Independent review found three release blockers before the branch checkpoint: install could resolve with a partial shell cache and then replace the last working worker; the unauthenticated sign-in gate did not have explicit safe-area padding outside `.crm-shell`; and global `telephone=no` metadata would remove useful automatic phone links from existing plain-text client and booking numbers. All three findings are being fixed before the validation suite is repeated.
